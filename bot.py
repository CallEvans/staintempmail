...
async def secmail_list(box: dict) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(url, timeout=aiohttp.ClientTimeout(total=10)) as r:
            logger.info(f"1secmail list status: {r.status}")
            if r.status == 200:
                data = await r.json()
                logger.info(f"1secmail list result: {data}")
                return data
            return []
...
async def guerrilla_list(box: dict) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(GUERRILLA_API, params=params,
                         timeout=aiohttp.ClientTimeout(total=10)) as r:
            logger.info(f"Guerrilla list status: {r.status}")
            data = await r.json(content_type=None)
            logger.info(f"Guerrilla list result: {data}")
            raw  = data.get("list", [])
...
async def mailtm_list(box: dict) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{MAILTM_API}/messages", headers=headers,
                         timeout=aiohttp.ClientTimeout(total=10)) as r:
            logger.info(f"mail.tm list status: {r.status}")
            data = await r.json(content_type=None)
            logger.info(f"mail.tm list result: {data}")
...
async def _is_member(user_id: int, bot) -> bool:
    ...
    try:
        ...
        # If check fails (e.g. bot not admin), let the user through
        # so they are not permanently locked out
        return True
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner-only: test all 3 APIs and show raw responses."""
    user = update.effective_user
    await update.message.reply_text("🔍 Testing all 3 mail APIs...")
    ...
    try:
        async with aiohttp.ClientSession() as s:
            ...
            async with s.get(url, timeout=aiohttp.ClientTimeout(total=8)) as r:
                results.append(f"✅ *1secmail* — HTTP {r.status}")
    except Exception as e:
        results.append(f"❌ *1secmail* — {type(e).__name__}: {e}")
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                results.append(f"✅ *Guerrilla Mail* — HTTP {r.status} · addr: `{email}`")
    except Exception as e:
        results.append(f"❌ *Guerrilla Mail* — {type(e).__name__}: {e}")
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                results.append(f"✅ *mail.tm* — HTTP {r.status} · domains: {domains}")
    except Exception as e:
        results.append(f"❌ *mail.tm* — {type(e).__name__}: {e}")

    # Show active inbox if any
    box = inboxes.get(user.id)
    if box:
        results.append(
            f"\n📬 *Your active inbox:*\n"
            f"`{box['login']}@{box['domain']}`\n"
            f"Service: {box['service']}\n"
            f"Token: `{'set' if box.get('token') else 'none'}`"
        )
    else:
        results.append("\n📭 No active inbox")
